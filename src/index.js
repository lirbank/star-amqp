...
const send = async function (q, data, confirm = true) {
  ...
  if (producerChannelCreating) {
    ...
    return await send(q, data, confirm);
  }
  ...
  if (confirm) {
    let confirmations = await waitForConfirms();
...
const waitForConfirms = async function () {
  return await producerChannel.waitForConfirms();
};

const registerWorker = async function (q, worker, prefetch) {
...
export class Queue {
  constructor(queue, options, worker = null) {
    ...
    this.queue = queue;
    this.worker = worker;
    this.done = registerWorker(queue, worker, prefetch || 1);
  }
  ...
  send(message, confirm = true) {
    return send(this.queue, message, confirm);
  }
}
